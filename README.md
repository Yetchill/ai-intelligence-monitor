...
和真实 AI 功能尚未实现。当前来源页面不允许新增、删除或任意编辑 `collector_config`。
...
然后访问 `http://127.0.0.1:8000/`。页面可以搜索和筛选资讯、收藏、人工修改分类、启停来源、
更新全部 enabled 来源或单个 enabled 来源，并查看运行记录。启动只检查 Alembic 迁移状态，不会
