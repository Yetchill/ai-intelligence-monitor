...
`=`、`+`、`-`、`@`、制表符或换行开头的内容会加文本前缀，防止 Excel 公式注入。文件不包含
外部图片、网页内容、附件或宏。
...
没有内容的分类不生成章节。每条资讯包含标题、来源、发布时间、人工/自动分类标记、可选简介
...
只有指定 `--force` 才原子替换。Excel 路径必须以 `.xlsx` 结尾，Word 路径必须以 `.docx` 结尾。
...
- Repository 先执行相同条件的计数，再执行带 `LIMIT` 的联表查询，不读取全库后由 Python 筛选；
