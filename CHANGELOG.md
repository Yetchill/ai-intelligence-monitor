...
- 补齐已有数据库的正式来源初始化路径：新增 `sources seed-formal` 与来源页幂等入口，并仅在阶段七 AIIA 受管配置完全未修改时安全提升且保留停用状态；
- 批量与定时更新默认只选择 enabled formal，保存来源支持不落库运行时预览，test/fallback 保留显式运行路径；
- 将 fetch/解析、配置、规范化、分类和持久化失败与内容准入拒绝分开统计和展示，不再把失败显示为 reject；
- 为资讯持久化准入标记，历史内容保留可查但不会因来源提升绕过准入进入默认首页或正式导出；
